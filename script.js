...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Chart Initialization (Only if on landing page)
...
function initCharts() {
    ...
    // (Other charts like MonkSalary, SocialSecurity, Membership removed to keep file concise if not needed or can be added back if requested)
}
...
// 🟢 NEW: EXECUTIVE & PARTY INFO FUNCTIONS
...
function renderPartyInfo(info) {
    ...
    update(dashIds.n, fmt(info.member_north));
    update(dashIds.c, fmt(info.member_central));
    update(dashIds.ne, fmt(info.member_ne));
    update(dashIds.s, fmt(info.member_south));
...
window.openModal = (type, record = null) => {
    if (type === 'edit' && record && typeof record === 'string') {
        ...
        if (r && r.type === 'feedback') { openFeedbackManageModal(r); return; }
    } else if (type === 'edit' && record && record.type === 'feedback') { openFeedbackManageModal(record); return; }
...
async function saveData(e) {
    ...
    finally { saveBtn.disabled = false; saveBtn.innerText = "บันทึกข้อมูล"; }
}
